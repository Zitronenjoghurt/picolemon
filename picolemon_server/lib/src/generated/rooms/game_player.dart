...
import 'package:picolemon_server/src/generated/protocol.dart' as _i3;
...
abstract class GamePlayer
  ...
  static GamePlayerInclude include({_i2.GameRoomInclude? room}) {
    return GamePlayerInclude._(room: room);
  }
...
class GamePlayerUpdateTable extends _i1.UpdateTable<GamePlayerTable> {
  ...
  _i1.ColumnValue<int, int> seat(int value) => _i1.ColumnValue(
    table.seat,
    value,
  );
...
class GamePlayerTable extends _i1.Table<int?> {
  GamePlayerTable({super.tableRelation}) : super(tableName: 'game_player') {
  ...
  _i2.GameRoomTable? _room;

  late final _i1.ColumnUuid userId;

  late final _i1.ColumnInt seat;

  late final _i1.ColumnDateTime joinedAt;

  _i2.GameRoomTable get room {
    if (_room != null) return _room!;
    _room = _i1.createRelationTable(
      relationFieldName: 'room',
      field: GamePlayer.t.roomId,
      foreignField: _i2.GameRoom.t.id,
      tableRelation: tableRelation,
      createTable: (foreignTableRelation) =>
          _i2.GameRoomTable(tableRelation: foreignTableRelation),
    );
    return _room!;
  }
...
class GamePlayerInclude extends _i1.IncludeObject {
  GamePlayerInclude._({_i2.GameRoomInclude? room}) {
    _room = room;
  }

  _i2.GameRoomInclude? _room;
...
class GamePlayerAttachRowRepository {
  ...
  /// Creates a relation between the given [GamePlayer] and [GameRoom]
  /// by setting the [GamePlayer]'s foreign key `roomId` to refer to the [GameRoom].
  Future<void> room(
    _i1.DatabaseSession session,
    ...
    var $gamePlayer = gamePlayer.copyWith(roomId: room.id);
    await session.db.updateRow<GamePlayer>(
      $gamePlayer,
      columns: [GamePlayer.t.roomId],
      transaction: transaction,
    );
  }
}
